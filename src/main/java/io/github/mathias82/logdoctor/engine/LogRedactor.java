...
public final class LogRedactor {
    ...
    private static final Pattern SECRET_ASSIGNMENT = Pattern.compile(
            "(?i)\\b(password|passwd|pwd|secret|api[_-]?key|access[_-]?token|refresh[_-]?token|client[_-]?secret)\\b(\\s*[:=]\\s*)([^\\s,;]+)");
    private static final Pattern QUERY_SECRET = Pattern.compile(
            "(?i)([?&](?:token|access_token|api_key|key|secret|password)=)[^&#\\s]+", Pattern.MULTILINE);
