...
public final class LogBatchAnalyzer {
    ...
    public BatchDiagnosisResult analyze(String log) {
        ...
        for (FailureBlock block : blocks.stream().limit(MAX_INCIDENT_BLOCKS).toList()) {
            ...
            groups.computeIfAbsent(fingerprint, ignored -> new MutableGroup(result, block.text(), fingerprint))
    ...
    private void enrichUnknownGroups(Map<String, MutableGroup> groups) {
        for (MutableGroup group : groups.values()) {
            if (!"UNKNOWN".equals(group.sample().status())) {
                continue;
            }
            var enriched = engine.analyzeStructured(group.representativeLog(), true);
            if (!"NO_FAILURE".equals(enriched.status())) {
                group.replaceSample(enriched);
            }
        }
    }
    ...
    private static boolean looksLikeNonFailureLogBoundary(String line) {
        ...
        String level = logLevel(line);
        return "TRACE".equals(level) || "DEBUG".equals(level) || "INFO".equals(level) || "WARN".equals(level);
    }
    ...
    private static ParsedTimestamp parseTimestamp(String text) {
        ...
        if (offsetMatcher.find()) {
            ...
            try {
                ...
                // Try local timestamp parsing next.
            }
        }
    ...
    private static List<String> investigationOrder(List<IncidentGroup> incidents) {
        return incidents.stream()
                .sorted(Comparator.comparingInt((IncidentGroup group) -> severityRank(group.severity())).reversed()
    ...
    private static List<RootCauseChain> rootCauseChains(List<IncidentCorrelation> correlations, List<IncidentGroup> incidents) {
        ...
        return correlations.stream()
                .map(correlation -> {
                    int from = severityByType.getOrDefault(correlation.fromType(), 0);
                    int to = severityByType.getOrDefault(correlation.toType(), 0);
                    int score = Math.min(100, 30 + correlation.occurrences() * 15 + from * 5 + to * 10);
    ...
    private static List<IncidentSpike> spikes(List<DiagnosedEvent> events) {
        ...
        for (DiagnosedEvent event : events) {
            ...
            if (minute == null || basis == null) {
                continue;
            }
            series.computeIfAbsent(event.fingerprint() + "|" + basis, ignored -> new SpikeSeries(event.type()))
                    .add(minute, event.timestamp().raw());
        }
        return series.values().stream()
                .map(SpikeSeries::detect)
    ...
    private static String reportMarkdown(
        ...
        if (investigationOrder.isEmpty()) {
            out.append("No failure detected.\n\n");
        } else {
            for (int i = 0; i < investigationOrder.size(); i++) {
                out.append(i + 1).append(". ").append(investigationOrder.get(i)).append('\n');
            }
        }

        out.append("\n## Root-cause chain candidates\n\n");
        if (chains.isEmpty()) {
            out.append("No timestamp-supported chain candidate detected.\n");
        } else {
            for (RootCauseChain chain : chains) {
                out.append("- **").append(chain.fromType()).append(" → ").append(chain.toType())
                        .append("** — score ").append(chain.score()).append("/100, ")
                        .append(chain.confidence()).append(", observed ").append(chain.occurrences()).append("×\n");
            }
        }

        out.append("\n## Spikes\n\n");
        if (spikes.isEmpty()) {
            out.append("No significant per-minute spike detected.\n");
        } else {
            for (IncidentSpike spike : spikes) {
                out.append("- **").append(spike.type()).append("** — ").append(spike.count())
                        .append(" events near ").append(spike.windowStart()).append(", ")
                        .append(String.format(Locale.ROOT, "%.1fx", spike.multiplier()))
                        .append(" baseline, score ").append(spike.score()).append("/100\n");
            }
        }
        ...
        for (IncidentGroup incident : incidents) {
            out.append("### ").append(incident.count()).append("× ").append(incident.type()).append("\n\n")
                    ...
                    .append("- Match strength: ").append(incident.matchScore()).append("/100 (")
                    .append(incident.matchConfidence()).append(")\n")
    ...
    private static final class MutableGroup {
        ...
        private MutableGroup(DiagnosisEngine.DiagnosisResult sample, String representativeLog, String fingerprint) {
            this.sample = sample;
        ...
        private void replaceSample(DiagnosisEngine.DiagnosisResult enriched) {
            sample = enriched;
        }
    ...
    private static final class MutableCorrelation {
        ...
        private IncidentCorrelation snapshot() {
            return new IncidentCorrelation(
                    fromType,
                    toType,
                    occurrences,
    ...
    private static final class SpikeSeries {
        ...
        private SpikeSeries(String type) {
            this.type = type;
        }
        ...
        private IncidentSpike detect() {
            if (buckets.size() < 2 || maxMinute <= minMinute) {
                return null;
            }
            Map.Entry<Long, MinuteBucket> peak = buckets.entrySet().stream()
                    .max(Map.Entry.comparingByValue(Comparator.comparingInt(bucket -> bucket.count)))
                    .orElseThrow();
            long span = maxMinute - minMinute + 1;
            double baseline = total / (double) span;
            double multiplier = baseline == 0 ? 0 : peak.getValue().count / baseline;
            if (peak.getValue().count < 3 || multiplier < 2.0) {
                return null;
            }
    ...
    private static final class MinuteBucket {
        ...
        private MinuteBucket(String raw) {
            this.raw = raw;
        }
    }
    ...
    private record ParsedTimestamp(String raw, Instant instant, LocalDateTime localDateTime) {
        ...
        private static ParsedTimestamp local(String raw, LocalDateTime local) {
            return new ParsedTimestamp(raw, null, local);
        }

        private String basis() {
            return instant != null ? "OFFSET" : localDateTime != null ? "LOCAL" : null;
        }
        ...
        private Long minuteIndex() {
            ...
            if (localDateTime != null) {
                return ChronoUnit.MINUTES.between(LocalDateTime.of(1970, 1, 1, 0, 0), localDateTime);
            }
            return null;
        }
    }
