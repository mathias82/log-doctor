...
public final class LogBatchAnalyzer {
    ...
    public LogBatchAnalyzer(DiagnosisEngine engine) { this.engine = engine; }
    ...
    public BatchDiagnosisResult analyze(String log) {
        ...
        List<IncidentGroup> incidents = groups.values().stream().map(MutableGroup::snapshot)
                .sorted(Comparator.comparingInt(IncidentGroup::count).reversed()).toList();
        ...
        String report = reportMarkdown(countLines(log), diagnosedEvents.size(), detectedFailureBlocks, truncated,
                incidents, investigationOrder, rootCauseChains, spikes);
        return new BatchDiagnosisResult(countLines(log), diagnosedEvents.size(), detectedFailureBlocks, truncated, incidents.size(),
    ...
    private static List<FailureBlock> splitFailureBlocks(String log) {
        String[] lines = log.split("\\R", -1); List<FailureBlock> blocks = new ArrayList<>();
        StringBuilder current = null; ParsedTimestamp currentTimestamp = null;
        for (String line : lines) {
            if (current == null) {
                if (looksLikeFailureStart(line)) { current = new StringBuilder(line); currentTimestamp = parseTimestamp(line); }
                continue;
            }
            if (looksLikeTopLevelFailureStart(line)) {
                blocks.add(new FailureBlock(current.toString(), currentTimestamp)); current = new StringBuilder(line); currentTimestamp = parseTimestamp(line);
            } else if (looksLikeNonFailureLogBoundary(line)) {
                blocks.add(new FailureBlock(current.toString(), currentTimestamp)); current = null; currentTimestamp = null;
            } else current.append(System.lineSeparator()).append(line);
        }
    ...
    private static boolean looksLikeFailureStart(String line) {
        ...
        return looksLikeTopLevelFailureStart(line) || lower.contains(" exception") || lower.startsWith("exception")
                || lower.contains("caused by:") || lower.contains("outofmemoryerror") || lower.contains("sockettimeoutexception") || lower.contains("deadlock");
    }
    private static boolean looksLikeTopLevelFailureStart(String line) {
        String lower = line.stripLeading().toLowerCase(Locale.ROOT);
        if (lower.startsWith("error ") || lower.startsWith("error:") || lower.startsWith("fatal ") || lower.startsWith("fatal:")) return true;
        if (parseTimestamp(line) == null) return false;
        String level = logLevel(line); return "ERROR".equals(level) || "FATAL".equals(level);
    }
    private static boolean looksLikeNonFailureLogBoundary(String line) {
        ...
        String level = logLevel(line); return "TRACE".equals(level) || "DEBUG".equals(level) || "INFO".equals(level) || "WARN".equals(level);
    }
    private static String logLevel(String line) { return LogLevelParser.parse(line); }
    private static boolean looksLikeContinuation(String line) {
        String trimmed = line.stripLeading(); return trimmed.startsWith("at ") || trimmed.startsWith("...") || trimmed.startsWith("Caused by:") || trimmed.startsWith("Suppressed:") || line.isBlank();
    }
    ...
    private static ParsedTimestamp parseTimestamp(String text) {
        ...
        if (offsetMatcher.find()) {
            ...
            try { return ParsedTimestamp.offset(raw, OffsetDateTime.parse(normalizeOffset(raw)).toInstant()); }
            catch (DateTimeParseException ignored) { }
        }
        Matcher localMatcher = LOCAL_TIMESTAMP.matcher(text);
        if (localMatcher.find()) {
            String raw = localMatcher.group(1), normalized = raw.replace('T', ' ').replace(',', '.');
            try { return ParsedTimestamp.local(raw, LocalDateTime.parse(normalized, LOCAL_TIMESTAMP_FORMATTER)); }
            catch (DateTimeParseException ignored) { return new ParsedTimestamp(raw, null, null); }
        }
        return null;
    }
    ...
    private static String normalizeRootCause(String value) {
        ...
        return UUID.matcher(value.toLowerCase(Locale.ROOT)).replaceAll("<uuid>").replaceAll("0x[0-9a-f]+", "<hex>")
                .replaceAll("\\b\\d+\\b", "<n>").replaceAll("\\s+", " ").trim();
    }
    private static int countLines(String log) { return log.isEmpty() ? 0 : log.split("\\R", -1).length; }
    private static List<String> investigationOrder(List<IncidentGroup> incidents) {
        return incidents.stream().sorted(Comparator.comparingInt((IncidentGroup g) -> severityRank(g.severity())).reversed()
                .thenComparing(Comparator.comparingInt(IncidentGroup::count).reversed())).limit(5).map(g -> g.type() + " — " + g.summary()).toList();
    }
    private static List<IncidentCorrelation> correlations(List<DiagnosedEvent> events) {
        ...
        return found.values().stream().map(MutableCorrelation::snapshot).sorted(Comparator.comparingInt(IncidentCorrelation::occurrences).reversed()).limit(10).toList();
    }
    private static List<RootCauseChain> rootCauseChains(List<IncidentCorrelation> correlations, List<IncidentGroup> incidents) {
        ...
        for (IncidentGroup incident : incidents) severityByType.merge(incident.type(), severityRank(incident.severity()), Math::max);
        return correlations.stream().map(c -> {
            int from = severityByType.getOrDefault(c.fromType(), 0), to = severityByType.getOrDefault(c.toType(), 0);
            int score = Math.min(100, 30 + c.occurrences() * 15 + from * 5 + to * 10);
            String confidence = score >= 80 ? "HIGH" : score >= 60 ? "MEDIUM" : "LOW";
            return new RootCauseChain(c.fromType(), c.toType(), c.occurrences(), score, confidence,
                    "Candidate dependency chain scored from temporal adjacency, repetition and incident severity; not proven causation.");
        }).sorted(Comparator.comparingInt(RootCauseChain::score).reversed().thenComparing(Comparator.comparingInt(RootCauseChain::occurrences).reversed())).limit(10).toList();
    }
    private static List<IncidentSpike> spikes(List<DiagnosedEvent> events) {
        ...
        for (DiagnosedEvent event : events) {
            if (event.timestamp() == null) continue; Long minute = event.timestamp().minuteIndex(); String basis = event.timestamp().basis();
            if (minute == null || basis == null) continue;
            series.computeIfAbsent(event.fingerprint() + "|" + basis, ignored -> new SpikeSeries(event.type())).add(minute, event.timestamp().raw());
        }
        return series.values().stream().map(SpikeSeries::detect).filter(s -> s != null).sorted(Comparator.comparingInt(IncidentSpike::score).reversed()
                .thenComparing(Comparator.comparingDouble(IncidentSpike::multiplier).reversed())).limit(10).toList();
    }
    private static boolean withinCorrelationWindow(DiagnosedEvent first, DiagnosedEvent second) {
        Long seconds = secondsBetween(first.timestamp(), second.timestamp()); return seconds != null && seconds >= 0 && seconds <= CORRELATION_WINDOW_SECONDS;
    }
    ...
    private static int severityRank(String severity) {
        return switch (severity == null ? "" : severity.toUpperCase(Locale.ROOT)) { case "CRITICAL" -> 4; case "HIGH", "ERROR" -> 3; case "MEDIUM", "WARN" -> 2; case "LOW" -> 1; default -> 0; };
    }

    private static String reportMarkdown(int totalLines, int failureBlocks, int detectedFailureBlocks, boolean truncated,
                                         List<IncidentGroup> incidents, List<String> investigationOrder, List<RootCauseChain> chains, List<IncidentSpike> spikes) {
        ...
        if (chains.isEmpty()) out.append("No timestamp-supported chain candidate detected.\n"); else for (RootCauseChain chain:chains) out.append("- **").append(chain.fromType()).append(" → ").append(chain.toType()).append("** — score ").append(chain.score()).append("/100, ").append(chain.confidence()).append(", observed ").append(chain.occurrences()).append("×\n");
        out.append("\n## Spikes\n\n");
        if (spikes.isEmpty()) out.append("No significant per-minute spike detected.\n"); else for (IncidentSpike spike:spikes) out.append("- **").append(spike.type()).append("** — ").append(spike.count()).append(" events near ").append(spike.windowStart()).append(", ").append(String.format(Locale.ROOT,"%.1fx",spike.multiplier())).append(" baseline, score ").append(spike.score()).append("/100\n");
        out.append("\n## Incident groups\n\n");
        for (IncidentGroup incident:incidents) {
            out.append("### ").append(incident.count()).append("× ").append(incident.type()).append("\n\n")
                    ...
                    .append("- Confidence: ").append(incident.confidence()).append('\n').append("- Match strength: ").append(incident.matchScore()).append("/100 (").append(incident.matchConfidence()).append(")\n")
    ...
    private static final class MutableGroup {
        ...
        private void addOccurrence(ParsedTimestamp timestamp) {
            ...
            Long fromFirst=secondsBetween(firstSeen,timestamp); if(fromFirst!=null&&fromFirst<0)firstSeen=timestamp;
    ...
    private static final class MutableCorrelation {
        private final String fromType,toType; private int occurrences; private MutableCorrelation(String fromType,String toType){this.fromType=fromType;this.toType=toType;} private void increment(){occurrences++;}
        private IncidentCorrelation snapshot(){return new IncidentCorrelation(fromType,toType,occurrences,"Observed consecutively within "+CORRELATION_WINDOW_SECONDS+" seconds using comparable timestamps");}
    }
    private static final class SpikeSeries {
        private final String type; private final Map<Long,MinuteBucket>buckets=new HashMap<>(); private int total; private long minMinute=Long.MAX_VALUE,maxMinute=Long.MIN_VALUE;
        private SpikeSeries(String type){this.type=type;} private void add(long minute,String raw){total++;minMinute=Math.min(minMinute,minute);maxMinute=Math.max(maxMinute,minute);buckets.computeIfAbsent(minute,i->new MinuteBucket(raw)).count++;}
        private IncidentSpike detect(){if(buckets.size()<2||maxMinute<=minMinute)return null; Map.Entry<Long,MinuteBucket>peak=buckets.entrySet().stream().max(Map.Entry.comparingByValue(Comparator.comparingInt(b->b.count))).orElseThrow(); long span=maxMinute-minMinute+1; double baseline=total/(double)span,multiplier=baseline==0?0:peak.getValue().count/baseline; if(peak.getValue().count<3||multiplier<2.0)return null; int score=Math.min(100,(int)Math.round(25+peak.getValue().count*8+multiplier*10)); return new IncidentSpike(type,peak.getValue().raw,peak.getValue().count,baseline,multiplier,score);}
    }
    private static final class MinuteBucket { private final String raw; private int count; private MinuteBucket(String raw){this.raw=raw;} }
    private record FailureBlock(String text,ParsedTimestamp timestamp){}
    private record ParsedTimestamp(String raw,Instant instant,LocalDateTime localDateTime){private static ParsedTimestamp offset(String raw,Instant instant){return new ParsedTimestamp(raw,instant,null);}private static ParsedTimestamp local(String raw,LocalDateTime local){return new ParsedTimestamp(raw,null,local);}private String basis(){return instant!=null?"OFFSET":localDateTime!=null?"LOCAL":null;}private Long minuteIndex(){if(instant!=null)return instant.getEpochSecond()/60;if(localDateTime!=null)return ChronoUnit.MINUTES.between(LocalDateTime.of(1970,1,1,0,0),localDateTime);return null;}}
    private record DiagnosedEvent(String fingerprint,String type,String severity,ParsedTimestamp timestamp){}

    public record BatchDiagnosisResult(int totalLines,int failureBlocks,int detectedFailureBlocks,boolean truncated,int uniqueIncidents,List<IncidentGroup> incidents,List<String> investigationOrder,List<IncidentCorrelation> correlations,List<RootCauseChain> rootCauseChains,List<IncidentSpike> spikes,String reportMarkdown){}
    public record IncidentGroup(String fingerprint,int count,String type,String category,String severity,String confidence,String summary,String rootCause,String location,String fixType,String fix,boolean humanReviewRequired,boolean llmUsed,String evidence,String firstSeen,String lastSeen,List<CauseChainAnalyzer.Cause> causeChain,List<String> matchReasons,int matchScore,String matchConfidence,List<String> matchScoreFactors){}
