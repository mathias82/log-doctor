...
public final class LogDoctorWebServer {
    ...
    private static void handleAnalyze(HttpExchange exchange) throws IOException {
        ...
        try {
            ...
            writeJson(exchange, 200, new DiagnosisEngine().analyzeStructured(request.log()));
        } catch (Exception e) {
    ...
    private static void handleStatic(HttpExchange exchange) throws IOException {
        if (!"GET".equalsIgnoreCase(exchange.getRequestMethod())) {
            writeText(exchange, 405, "Method not allowed", "text/plain; charset=utf-8"); return;
        }
        ...
        try (InputStream in = LogDoctorWebServer.class.getResourceAsStream(resource)) {
            if (in == null) { writeText(exchange, 404, "Not found", "text/plain; charset=utf-8"); return; }
            String contentType = resource.endsWith(".html") ? "text/html; charset=utf-8"
                    : resource.endsWith(".css") ? "text/css; charset=utf-8" : "application/javascript; charset=utf-8";
