...
public record RemediationPlaybook(
    ...
    static RemediationPlaybook forIncident(String incidentType, IncidentCategory category) {
    ...
    private static RemediationPlaybook kafka(String type) {
        ...
        return new RemediationPlaybook(
                List.of("Kafka client, broker and topic telemetry around the failure"),
                List.of("Apply only the configuration or operational change supported by collected evidence"),
                List.of("Replay the same workload pattern and confirm stable Kafka behavior"),
                List.of("Broker/controller instability or unclear data-loss implications")
        );
    }
