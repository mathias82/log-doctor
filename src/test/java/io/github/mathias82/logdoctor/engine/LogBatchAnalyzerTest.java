...
class LogBatchAnalyzerTest {
    ...
    @Test
    void exposesMatchAndCauseMetadataOnGroupedIncident() {
        String log = """
                2026-09-01 14:32:17 ERROR request failed
        ...
        assertThat(incident.matchScore()).isGreaterThan(0);
        assertThat(incident.matchConfidence()).isNotBlank();
        assertThat(incident.matchReasons()).isNotEmpty();
        assertThat(incident.matchScoreFactors()).isNotEmpty();
        assertThat(incident.causeChain()).isNotEmpty();
        assertThat(incident.causeChain().getFirst().exceptionType()).contains("NullPointerException");
    }

    @Test
    void callsLlmOnceForRepeatedUnknownIncidentGroup() {
        CountingLlmClient llm = new CountingLlmClient();
        LogBatchAnalyzer batchAnalyzer = new LogBatchAnalyzer(new DiagnosisEngine(llm));
        String log = """
                2026-09-01 14:32:17 ERROR request failed
                java.lang.RuntimeException: order 123 failed
                at com.acme.OrderService.run(OrderService.java:41)
        ...
        var result = batchAnalyzer.analyze(log);
